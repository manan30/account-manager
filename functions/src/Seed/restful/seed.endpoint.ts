...
import {
  generateFakeCreditor,
  generateFakeSpending
...
expressApp.post('/', async (req, res) => {
  ...
  try {
    ...
    if (seedOptions.spending) {
      const { clear: clearSpending, count } = seedOptions.spending;
      const spendingDbRef = db.collection(SpendingCollection);

      const spendingDbSnapshot = await spendingDbRef.get();

      if (clearSpending) {
        const batch = db.batch();
        spendingDbSnapshot.docs.forEach((doc) => {
          batch.delete(doc.ref);
        });
        await batch.commit();
      }

      const spendingCount = count ?? 10;
      const batch = db.batch();

      for (let i = 0; i < spendingCount; i += 1) {
        const creditor = generateFakeSpending();
        const id = db.collection(SpendingCollection).doc().id;
        const docRef = db.collection(SpendingCollection).doc(id);
        batch.set(docRef, creditor);
      }
